"""
Fiji / ImageJ external tool bridge.

Provides:
  - FijiBackendMixin  — backend detection, pyimagej init, version compat
  - FijiBridge        — coordinator that delegates to OrientationJBridge and
                        RidgeDetectionBridge
  - OrientationJBridge — OrientationJ plugin interface (pyimagej / subprocess / NumPy)
  - RidgeDetectionBridge — Ridge Detection plugin interface (pyimagej / subprocess / NumPy)
  - normalise_image   — float32 [0,1] normalisation helper
...
This module belongs in ``tme_quant.integrations`` because Fiji/ImageJ is an
external tool that tme_quant calls **out to** (subprocess or pyimagej).  It is
not a fiber analysis method in its own right; ``OrientationJMethod`` and
``RidgeDetectionMethod`` in ``fiber_analysis.methods`` use it as a backend.

Planned sibling modules in this package:
  qupath_bridge  — GeoJSON batch export for QuPath import/annotation
  matlab_bridge  — MATLAB Engine bridge (legacy curvelet support)

Note: napari is NOT included here.  The dependency arrow is reversed — napari
calls INTO tme_quant.  Napari-specific code lives in the napari-tme-quant
plugin package, never in this library.
"""

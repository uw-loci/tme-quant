...
class RegistrationManager:
    ...
    def _preprocess_images(
        ...
        from ..preprocessing import intensity_normalization, modality_conversion
        
        # Convert to grayscale if needed
        if fixed.ndim == 3:
            fixed = modality_conversion.rgb_to_grayscale(fixed)
        if moving.ndim == 3:
            moving = modality_conversion.rgb_to_grayscale(moving)
        
        # Normalize intensities
        if params.normalize_intensity:
            fixed = intensity_normalization.normalize_image(fixed)
            moving = intensity_normalization.normalize_image(moving)
        
        # Histogram matching
        if params.histogram_matching:
            moving = intensity_normalization.match_histograms(moving, fixed)

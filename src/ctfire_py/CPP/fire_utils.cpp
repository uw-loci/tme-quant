...
py::list fiberproc_native(int nPt, py::array_t<int32_t> X_in, py::list F_v_in, std::vector<int> nucleation_pts, float thresh_linka, int sp);
...
PYBIND11_MODULE(fiber_backend, m) {
    ...
    m.def("link_fibers", &fiberproc_native, "Link fiber segments based on nucleation points");

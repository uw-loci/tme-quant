...
def display_image_comparison(original_img, overlay_path, procmap_path=None):
    ...
    try:
        ...
        fig, axes = plt.subplots(
            1, n_plots, figsize=(fig_width, fig_height), sharex=True, sharey=True
        )
        ...
        if procmap_img is not None:
            ...
            ax3.set_title(
                "Processed Map (Heatmap)", fontsize=14, fontweight="bold", pad=10
            )
    ...
    try:
        ...
        for sheet_idx, sheet_name in enumerate(excel_file.sheet_names):
            ...
            if len(df) > 0:
                ...
                table_widget.setVerticalScrollBarPolicy(
                    Qt.ScrollBarPolicy.ScrollBarAsNeeded
                    if QT_VERSION == 6
                    else Qt.ScrollBarAsNeeded
                )
...
class ProcessImageGUI(QMainWindow):
    ...
    def init_widgets(self):
        ...
        self.fiber_mode.addItems(
            ["0 - Curvelet", "1 - FIRE v1", "2 - FIRE v2", "3 - FIRE v3"]
        )
        ...
        self.tif_boundary.addItems(
            ["0 - None", "1 - CSV Type 1", "2 - CSV Type 2", "3 - TIFF"]
        )
        self.boundary_img_path = QLineEdit()
        self.boundary_img_path.setPlaceholderText(
            "Required when Boundary Type = 3 - TIFF"
        )
    ...
    def browse_image(self):
        ...
        filename, _ = QFileDialog.getOpenFileName(
            self, "Select Image File", "", "TIFF files (*.tif *.tiff);;All files (*.*)"
        )
        if filename:
    ...
    def auto_detect_boundary_image(self):
        ...
        if os.path.exists(boundary_path):
            ...
            if os.path.exists(boundary_path_alt):
                ...
                print(
                    f"Boundary image not found at: {boundary_path} or {boundary_path_alt}"
                )
    ...
    def run_analysis(self):
        ...
        if not os.path.exists(self.img_path.text()):
            QMessageBox.critical(
                self, "Error", f"Image file not found: {self.img_path.text()}"
            )
            return
        ...
        try:
            ...
            if tif_boundary_val == 3 and self.boundary_img_path.text():
                if os.path.exists(self.boundary_img_path.text()):
                    ...
                    if boundary_img.ndim == 3:
                        if boundary_img.shape[2] == 3:
                            # RGB to grayscale
                            boundary_img = (
                                0.299 * boundary_img[:, :, 0]
                                + 0.587 * boundary_img[:, :, 1]
                                + 0.114 * boundary_img[:, :, 2]
                            )
                    ...
                    # Automatically set exclude_fibers_in_mask=1 when using TIFF boundary
                    exclude_fibers_flag = 1
                    print(
                        "Automatically set exclude_fibers_in_mask=1 for TIFF boundary"
                    )
                else:
                    QMessageBox.warning(
                        self,
                        "Warning",
                        f"Boundary image file not found: {self.boundary_img_path.text()}\nProceeding without boundary analysis.",
                    )
                ...
                QMessageBox.warning(
                    self,
                    "Warning",
                    "Boundary Type is set to '3 - TIFF' but no boundary image file is selected.\nProceeding without boundary analysis.",
                )
                tif_boundary_val = 0  # Reset to no boundary
            ...
            advanced_options = AdvancedAnalysisOptions(
                exclude_fibers_in_mask=exclude_fibers_flag,
            ...
            # Display results
            display_results(
                img, self.output_dir.text(), result, img_name=self.img_name.text()
            )
...
def main_cli():
    ...
    advanced_options = AdvancedAnalysisOptions(
        exclude_fibers_in_mask=False,  # Don't exclude fibers (no mask for tif_boundary=0)
